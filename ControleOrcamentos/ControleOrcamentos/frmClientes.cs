...
namespace ControleOrcamentos
{
    public partial class frmClientes : Form
    {
        ...
        private void dataGridViewPesquisar_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            // Ao selecionar o registro no grid, habilita na aba principal os dados para edição
            int count = dataGridViewPesquisar.Rows.Count;

            if (e.RowIndex >= 0 && e.RowIndex != count - 1)
            {
                //preenche os textbox com as células da linha selecionada
                DataGridViewRow row = dataGridViewPesquisar.Rows[e.RowIndex];
